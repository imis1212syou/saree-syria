...
(function(){
  ...
  function getVisitorId(){
    try{ let id=localStorage.getItem('saree_visitor_id'); if(!id){ id=(crypto.randomUUID ? crypto.randomUUID() : ('v_'+Date.now()+'_'+Math.random().toString(36).slice(2))); localStorage.setItem('saree_visitor_id',id); } return id; }
    catch(e){ return 'session_'+Date.now()+'_'+Math.random().toString(36).slice(2); }
  }
  window.recordStoreVisit = async function(storeId){
    if(!storeId) return; try{ await supabaseClient.from('store_visits').insert({store_id:storeId,visitor_id:getVisitorId()}); }catch(e){ console.warn('store visit:',e); }
  };
  async function recordCompanyVisit(companyId){
    if(!companyId) return; try{ await supabaseClient.from('company_visits').insert({company_id:companyId,visitor_id:getVisitorId()}); }catch(e){ console.warn('company visit:',e); }
  }
  ...
  async function renderCompanyDetail(name,list){
    if(!$('companyDetail')) return;
    $('companyDetailName').textContent = name || 'الشركة';
    const companyId = list.find(x=>x?.companies?.id)?.companies?.id || null;
    $('companyDetailBody').innerHTML = `
      <div class="card">
        <span class="pill companyBadge">🏢 ${esc(name || 'متاجر مستقلة')}</span>
        <div class="name">${list.length} متاجر</div>
        ${companyId ? `<div class="muted" style="margin-top:8px">إجمالي زيارات الشركة: <b id="publicCompanyVisits">—</b> • الفريدون: <b id="publicCompanyUniqueVisits">—</b></div>` : ''}
    ...
    if(companyId){ recordCompanyVisit(companyId); try{ const q=await supabaseClient.rpc('company_visit_counts',{p_company_id:companyId}); if(!q.error && q.data){ if($('publicCompanyVisits')) $('publicCompanyVisits').textContent=fmt(q.data.total_visits||0); if($('publicCompanyUniqueVisits')) $('publicCompanyUniqueVisits').textContent=fmt(q.data.unique_visitors||0); } }catch(e){ console.warn('company counts:',e); } }
    window.scrollTo(0,0);
  }
  ...
  window.renderMerchant = async function(){
    ...
    $('merchantPanel').innerHTML=`
      <div class="card">
        ...
        ${st ? `<div class="card" style="margin-top:10px"><div class="name" id="merchantVisitorTotal">—</div><div class="muted">إجمالي زيارات المتجر</div><div class="name" id="merchantVisitorCount" style="margin-top:8px">—</div><div class="muted">الزوار الفريدون</div></div>` : ''}
        <div class="actions">
  ...
  window.loadMerchantStoreVisitorCount = async function(){
    ...
    try{
      const {data,error}=await supabaseClient.rpc('merchant_store_visit_counts',{p_store_id:profileData.store_id});
      if(!error && data){ if($('merchantVisitorTotal')) $('merchantVisitorTotal').textContent=fmt(data.total_visits||0); if($('merchantVisitorCount')) $('merchantVisitorCount').textContent=fmt(data.unique_visitors||0); }
    }catch(err){ console.warn('merchant visitor count:',err); }
  };

  window.loadVisitorCount = async function(){
    if(!isAdmin()) return;
    try{ const [total,unique]=await Promise.all([supabaseClient.rpc('admin_visitor_count'),supabaseClient.rpc('admin_unique_visitor_count')]); if(!total.error && $('visitorTotalCount')) $('visitorTotalCount').textContent=fmt(total.data||0); if(!unique.error && $('visitorCount')) $('visitorCount').textContent=fmt(unique.data||0); }
  ...
  window.loadAdminStoreVisitorCounts = async function(){
    ...
    try{
      ...
      data.forEach(row=>{ const node=$('sv_'+row.store_id); if(node) node.textContent=fmt(row.unique_visitors ?? row.visitor_count ?? 0); const total=$('svt_'+row.store_id); if(total) total.textContent=fmt(row.total_visits||0); });
    }catch(err){ console.warn(err); }
  };

  window.loadAdminCompanyVisitorCounts = async function(){
    if(!isAdmin()) return;
    try{ const {data,error}=await supabaseClient.rpc('admin_company_visitor_counts'); if(error || !Array.isArray(data)) return; data.forEach(row=>{ const t=$('cvt_'+row.company_id); const u=$('cv_'+row.company_id); if(t) t.textContent=fmt(row.total_visits||0); if(u) u.textContent=fmt(row.unique_visitors||0); }); }catch(err){ console.warn('company visitor counts:',err); }
  };
  ...
  window.renderAdmin = async function(){
    ...
    $('adminPanel').innerHTML=`
      <div class="grid"><div class="card"><div class="name">${requests.length}</div><div class="muted">طلبات معلقة</div></div><div class="card"><div class="name">${(stores||[]).length}</div><div class="muted">متاجر</div></div><div class="card"><div class="name">${(products||[]).length}</div><div class="muted">منتجات</div></div><div class="card"><div class="name">${users.length}</div><div class="muted">حسابات</div></div><div class="card"><div class="name" id="visitorTotalCount">—</div><div class="muted">إجمالي زوار الموقع</div><div class="name" id="visitorCount" style="margin-top:8px">—</div><div class="muted">زوار الموقع الفريدون</div></div></div>
