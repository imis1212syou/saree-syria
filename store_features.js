...
(function(){
  ...
  window.adminAddStore = async function(){
    ...
    const payload={name,city:$('scity').value.trim(),area:$('sarea').value.trim(),address:$('saddr').value.trim(),phone:$('sphone').value.trim(),opening_hours:$('shours').value.trim(),working_days:$('sdays').value.trim(),image_url:image,whatsapp_url:$('swhatsapp').value.trim()||null,company_name:$('scompany').value.trim()||null,verified:true,active:true};
