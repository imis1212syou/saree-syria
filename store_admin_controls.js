...
(function(){
  ...
  const getProfile = () => { try { return (typeof profileData !== 'undefined' && profileData) ? profileData : window.profileData; } catch (_) { return window.profileData || null; } };
  ...
  const getAdminUid = () => { try { return (typeof ADMIN_UID !== 'undefined' ? ADMIN_UID : window.ADMIN_UID) || ''; } catch (_) { return window.ADMIN_UID || ''; } };
  const isAdmin = () => String(getProfile()?.role || '').toLowerCase() === 'admin';
  ...
  function storeById(id){
    return getStores().find(x=>String(x.id)===adminStoreId(id)) || null;
  }
  ...
  window.deleteAdminMerchant = async function(id){
    if(!adminOnly()) return;
    if(String(id)===String(getAdminUid())) return alert('لا يمكن حذف حساب المدير.');
    const profiles=await loadProfiles().catch(err=>{alert(err.message);return null});
    if(!profiles)return;
    const u=profiles.find(x=>String(x.id)===String(id));
    if(!u)return alert('الحساب غير موجود.');
