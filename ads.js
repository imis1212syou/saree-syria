(function(){
  ...
  window.openAdminAds=async function(){
    ...
    if(typeof overlay==='function') overlay(html); else alert('واجهة إدارة الإعلانات غير متاحة.');
  };
  ...
  window.openAdminAdCreate=function(){if(typeof profileData==='undefined'||profileData?.role!=='admin')return alert('هذا الخيار للمدير فقط.');overlay(adForm(null));};
  window.openAdminAdEdit=async function(id){
    if(typeof profileData==='undefined'||profileData?.role!=='admin')return alert('هذا الخيار للمدير فقط.');
    const {data,error}=await supabaseClient.from('ads').select('*').eq('id',id).maybeSingle();
    if(error)return alert(error.message);if(!data)return alert('الإعلان غير موجود.');overlay(adForm(data));
  };
  window.saveAdminAd=async function(id){
    ...
    const {error}=await q;if(error)return alert(error.message);alert(id?'تم تعديل الإعلان.':'تمت إضافة الإعلان.');closeAdminEditor();await window.refreshAds();await window.renderAdmin();
  };
